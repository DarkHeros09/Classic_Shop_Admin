...
class ColorRemoteService implements IColorRemoteService {
  ...
  Future<RemoteResponse<List<ColorDTO>>> fetchColors({
    ...
    try {
      ...
      if (response.isSuccessful) {
        ...
        // final colorDtos = convertedBody.map(ColorDTO.fromJson).toList();
  ...
  Future<RemoteResponse<ColorDTO>> updateColor({
    required int adminId,
    required int productColorId,
    required String color,
  }) async {
    try {
      final response = await _colorAdminApi.updateColor(
        adminId: adminId.toString(),
        id: productColorId.toString(),
        data: {
          'color': color,
        },
      );
