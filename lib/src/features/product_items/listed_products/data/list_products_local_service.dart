...
class ListProductsLocalService {
  ListProductsLocalService(this._sembastDatabase);

  final SembastDatabase _sembastDatabase;
  // final _store = intMapStoreFactory.store('products');

  Future<void> upsertPage(
    List<ProductItemDTO> dtos,
    int page,
    String uri,
  ) async {
  ...
  Future<List<ProductItemDTO>> getPage(int page, String uri) async {
    debugPrint('ZXZX $uri');
    final store = intMapStoreFactory.store(uri);
    // final sembastPage = page - 1;
    ...
    return records.map((e) => ProductItemDTO.fromJson(e.value)).toList();
  }
