...
import 'package:classic_shop_admin/src/features/product/add_product/application/add_product_notifier.dart';
...
class _AddProductPageState extends ConsumerState<AddProductPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final categoryLoaded = useState(false);
    final brandSearchValue = useState('');
    final brandSearchKey = useState(0);
    final brandLoaded = useState(false);
    ...
    ref
      ..listen(
        ...
        (previous, next) => next.mapOrNull(
          loadSuccess: (_) => categoryLoaded.value = true,
        ),
      )
      ..listen(
        brandNotifierProvider,
        (previous, next) => next.mapOrNull(
          loadSuccess: (_) => brandLoaded.value = true,
        ),
      )
      ..listen(
    ...
    return categoryLoaded.value && brandLoaded.value
        ? Scaffold(
                              ...
                              options: [
                                ...
                                ...brandState.entries.map(
                                  (brand) {
                                    ...
                                    // because it keeps the widget in the tree.
                                    return Offstage(
                                      offstage: !filteredBrands()
                                          .containsKey(brand.key),
                                      child: ShadOption(
                                        value: brand.key,
                                ...
                                ShadButton(
                                  onPressed: () {
                                    ...
                                    if (validated != null &&
                                      ...
                                      ref
                                          .read(
                                            addProductNotifierProvider.notifier,
                                          )
                                          .createProduct(
                                            name: values!['name']!
                                                .value
